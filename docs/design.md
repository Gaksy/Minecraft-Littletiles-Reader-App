...
└── 项目列表（**一行一个项目**：封面 · 名称 · 描述 · 目录）
     ├── [新建项目] → **配置向导**（项目名 → 简介 → 存档目录（必填）→ 封面）
     ├── [添加已有项目] / [刷新]
     └── 点击卡片 → 项目工作界面（**删除项目在项目里**，不在卡片上）

项目管理界面（点进某个项目后，这个项目的一切都在这里）
├── 顶部：两个大区域 —— [导出 OBJ 模型] / [导出 SNBT]
│    └── 点"导出 OBJ" → 直接弹出"导出哪个区块"（§6）
│         └── 存档位置由项目配置决定，对话框里**只显示、不可改**（要改去「项目配置」）
...
├── 导出概览：这个项目**导过哪些区块**——以数据为中心向外 5 格，灰/绿/黄三态，
│    悬停看坐标、点一格看详情（时间 / 面数 / 产物 / 记录号），可拖拽平移；
│    点到有记录的区块可以直接「查看这个区块的记录」（跳到按坐标搜记录）
├── 搜索区块记录：给一个 (x, z)（可选维度），列出这个区块的每一次导出与产物；
│    「查询区块」那个范围网格已被它取代——概览图已经接管"看哪些块导过"
├── 存储：大小统计与分类占比（§7.8）、清理策略（§7.7）
├── 导入 / 导出：把项目配置导出成一个包（换机器或备份用），以及从包导入
└── 菜单「项目 → 删除项目」：仅从列表移除，或连目录一起删（后者要额外勾确认）

管理界面（从启动界面或项目界面进入）
├── 素材管理：导入材质包 + 命名 + 查看内容（方块数/贴图数/缺失）
├── 模组管理：导入 jar → 自动解析方块与材质
├── 输出目录管理：默认输出目录、各来源的输出目录
├── 项目管理：新建（向导）/删除/重定位、封面、描述、绑定关系
├── 帮助 → 检查更新（网站公开的发布接口）/ 反馈问题（匿名提交 + 自动附脱敏日志）
│    └── 服务器侧现状与"还缺什么"见 docs/update-and-feedback.md
...
界面语言：**简体中文（源语言）/ 繁體中文 / English / 日本語 / 한국어 / Deutsch /
Français**。菜单位置：`视图 → 语言`（外加"跟随系统"），选择写进 `AppConfig.language`，
**重启后生效**——Qt 控件里的文案是死值，整窗重建比逐个回填更不容易出现
"一半新语言、一半旧语言"的界面。

机制（`app/i18n.py` + `app/i18n_data/<语言>.py`）：

- **键是简体中文原文**（不是自编 id）。漏翻的地方原样显示中文，看得懂、能用；
  代价是改中文文案要顺带改词表，这条写在这里提醒。
- 两层覆盖：`tr()` 管带 `%s/%d` 的动态模板；`translate(widget)` 在建完界面后扫一遍
  控件树与菜单 `QAction`，管静态文案（按钮、标签、分组标题、占位文字、tooltip）。
  认不出的原样保留，所以路径、文件名、状态文字不会被误翻。
- 已知未覆盖：来自 `savefolder` 的存档检查提示、日志里的中文行、以及
  项目管理窗口里的长说明文字。

现在的覆盖：**界面控件 100%**（按钮 / 标签 / 分组标题 / 表头 / 下拉项 / 占位文字 /
tooltip / 菜单项）、弹窗的标题与正文、概览图与搜记录、导出对话框、素材管理、
新建向导、清空数据、删除项目。剩下的是**日志行**（诊断用，刻意不翻）与
`savefolder` 里"选错存档目录"的提示。

自检 `tests/test_i18n.py` 会真建 15 个窗口/弹窗，统计"英文界面下的中文残留"，
任何一处漏翻都会让它挂掉——这也是发现 `translate()` 对 QGroupBox 抛异常的原因
（中文界面下走不到那条分支，只有切语言才炸）。

自检 `tests/test_i18n.py` 拦三种错：键写错一个字（真建窗口统计残留）、空译文、
把中文抄一遍（拉丁字母语言一律不允许，汉字圈语言允许同形词但整表照抄会挂）。
...
（三种模式的示意图在界面里现画：`app/ui/chunk_modes_art.py`，
颜色取自当前主题，所以深浅两套都清楚；原来的固定配色 SVG 已删。）

**两处网格，别搞混**：

* **导出对话框**里的那块，画的是"**这次要导的范围**里哪些块导过没有"（三态配色），
  范围由输入框决定，图只是预览；
* **项目界面 → 导出概览**画的是"**这个项目导过哪些区块**"（以数据为中心向外 5 格，
  可拖拽、可点开详情）。要查"哪块导过"看这里。
