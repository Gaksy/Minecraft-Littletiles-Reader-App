...
> 状态（2026-09-14 更新）：方案已落地，**照着做的步骤在 [packaging-howto.md](packaging-howto.md)**——
...
> ⏳ 仍待办：第一次真机构建（PyInstaller 还没装）、CI（可选）。
> 已定：形态 B（库+客户端一个包）、数据目录=应用文件夹、先只做 macOS ARM、不签名（附自签名说明）。
