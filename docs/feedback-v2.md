...
> 本文是设计，**未实现**（除文末标注"已完成"的几项）。文末有 3 个待你拍板的决定。

## 1. 你的诉求 → 落点

| 诉求 | 落在哪儿 | 现状 |
|---|---|---|
| 后台按来源分两个 tab：网站反馈 / LittleTiles Reader 反馈 | 服务端加 `source`，页面加一排 scope tab | 待做 |
| 网站里 littletiles 模块的反馈也算进 LT | 网站的反馈表单加 `littletiles` 选项，LT 页面进入时默认选中 | 待做 |
...
| 软件里能查数据码、自动记录、启动自动查状态、已解决弹窗、查看后标已读 | 客户端「我的反馈」+ 启动静默检查 | 待做 |
| 惯用语言选项 + 回复双语（中文原文 + 用户语言对照） | 服务端 `locale` + 译文字段；后台双栏回复；客户端译文在前、可展开中文 | 待做 |
| 其它反馈模块同步这套 | 网站的反馈页与后台回复组件共用同一套字段与渲染 | 待做（随上面一起） |
