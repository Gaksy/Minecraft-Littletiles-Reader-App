...
| 方案 | 做法 | 成本 | 备注 |
|---|---|---|---|
| ~~A 人工粘贴~~ | ~~复制提示词、自己翻了粘回来~~ | 0 | 已不需要（B 已实现），但译文始终可手改 |
| **B 服务端接 LLM（已实现）** | 接 **DeepSeek**；key 在后台「系统管理 → AI 翻译设置」页面里配（存 `sys_config` 表），改完**立即生效、不用重启** | DeepSeek 按量计费 | 反馈回复与 LT 多语言文案共用同一套；接口只回掩码，key 不出服务端 |
...
**P2（已实现）**：AI 一键翻译（DeepSeek，后台页面配置：`系统管理 → AI 翻译设置`）。
