...
STRINGS = {
    ...
    '有新版本：%s → %s': 'New version: %s → %s',
    '已是最新版本（%s）': 'Up to date (%s)',
    '更新内容：\n%s': 'Update notes:\n%s',
    '只检查稳定版；测试版请到下载页自己取。': 'Only stable releases are checked; grab betas from the download page if you want them.',
    '服务器上目前只有测试版；客户端只检查稳定版更新。': 'Only a beta is published for this platform right now; the client only checks stable updates.',
    '服务器上还没有当前平台（%s）的发布记录': 'No release recorded for this platform (%s) yet',
    '当前版本：%s\n服务器版本：%s': 'Current: %s\nOn the server: %s',
    '更新日志与最低支持版本还没在服务器上；现在只提示版本号与下载地址。': 'Release notes and the minimum supported version are not on the server yet; only the version number and download link are shown.',
    '打开下载页': 'Open download page',
    '连不上服务器：%s\n\n可以稍后再试；检查更新的接口是公开的。': 'Cannot reach the server: %s\n\nTry again later; the update check endpoint is public.',
    '标题': 'Title',
    '类型': 'Type',
    '严重程度': 'Severity',
    '联系方式': 'Contact',
    ...
    '邮箱或其它联系方式（可选）': 'Email or other contact (optional)',
    '复现步骤、期望结果、实际结果（越具体越好）': 'Steps to reproduce, expected result, actual result (the more specific the better)',
    '附带诊断信息与日志尾部（推荐）': 'Attach diagnostics and the log tail (recommended)',
    '只包含版本、系统、日志尾部；已去掉本机用户名与绝对路径': 'Contains only versions, OS and the log tail; local user names and absolute paths are stripped',
    '将发送的内容（可以先看一眼）': 'What will be sent (take a look first)',
    '服务器上限 %d 字符，当前 %d（超了会被截断）': 'Server limit %d characters, currently %d (longer text is truncated)',
    '发送反馈': 'Send report',
    '复制数据码': 'Copy data code',
    '请先填标题。': 'Please fill in a title first.',
    '已提交：编号 %s，数据码 %s。\n凭数据码可以查进度；本机也留了一份：\n%s': 'Submitted: number %s, data code %s.\nUse the code to check progress; a local copy is kept at:\n%s',
    '数据码已复制：%s（查询进度用它）': 'Data code copied: %s (use it to check progress)',
    '网络不通': 'No network',
    '服务器拒绝了这次提交': 'The server rejected this submission',
    '服务器返回的内容看不懂': 'The server response was not understood',
    '内容有问题': 'Problem with the content',
    '%s：%s\n\n内容已留在本机，可以直接发邮件：\n%s': '%s: %s\n\nThe content was saved locally; you can send it by email:\n%s',
    '轻微': 'Minor',
    '一般': 'Normal',
    '严重': 'Serious',
    '致命': 'Critical',
    ...
    # ---- 我的反馈（状态追踪与双语回复）----
    '我的反馈': 'My reports',
    '反馈有新进展': 'Your report has an update',
    '你反馈的 %s 有结论了：%s': 'Your report %s has a conclusion: %s',
    '另有 %d 条也有新进展。': '%d other report(s) also have updates.',
    '查看': 'View',
    '以后再说': 'Later',
    '这里列出本机提交过的反馈；「查看详情」会去服务器查最新状态与回复。': 'Reports submitted from this computer. "View details" asks the server for the latest status and reply.',
    '未读': 'Unread',
    '未查询': 'Not checked yet',
    '状态': 'Status',
    '编号': 'Number',
    ...
    '处理方案': 'Resolution',
    '处理意见': 'Notes',
    '（本条回复只有中文）': '(This reply is Chinese only)',
    '查看中文原文（对照）': 'Show the Chinese original',
    '查看详情': 'View details',
    '打开本机日志': 'Open local log',
    '删除记录': 'Delete entry',
    '已复制': 'Copied',
    '数据码已复制：%s（到网站 /feedback 也能查）': 'Data code copied: %s (also trackable at /feedback on the site)',
    '只删本机这条记录？\n\n服务器上的反馈不会删，但数据码会丢，之后查不了进度。': 'Delete only this local entry?\n\nThe report on the server is not deleted, but you will lose the data code and can no longer track it.',
    '查询失败': 'Lookup failed',
